...
export default function PlayGame() {
  ...
  useEffect(() => {
    const loadFriends = async () => {
      setLoading(true); // <-- düzeltme: loading burada başlatılıyor
      await fetchFriends();
      setLoading(false); // <-- düzeltme: loading burada bitiriliyor
    };
    loadFriends();
  }, [fetchFriends]);
  if (loading) return <LoadingOverlay />; // <-- aynen bırakıldı, artık sorun çıkmıyor
  ...
  const getArrow = (
    ...
    return guessValue > secretValue ? (
      <span className="text-blue-600 font-bold text-lg">⬇️</span> // tahmin yüksekse aşağı
    ) : (
      <span className="text-red-600 font-bold text-lg">⬆️</span> // tahmin düşükse yukarı
    );
  };
